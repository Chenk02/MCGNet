...
class HDNet(nn.Module):
    ...
    def __init__(self, num_class, num_heading_bin, num_size_cluster, mean_size_arr,
        ...
        self.conv2d_1 = torch.nn.Conv2d(1024, 1024, 1)
        self.conv2d_2 = torch.nn.Conv2d(1024, 1024, 1)
    ...
    def forward(self, inputs, end_points, mode=""):
        ...
        features1_copy = torch.unsqueeze(features1, dim=1)
        features2_copy = torch.unsqueeze(features2, dim=1)
        features3_copy = torch.unsqueeze(features3, dim=1)
        features4_copy = torch.unsqueeze(features4, dim=1)

        all_features = torch.cat((features1_copy, features2_copy, features3_copy, features4_copy), dim=1)
        all_features = torch.transpose(all_features, 1, 3)
        all_features = self.conv2d_1(all_features)
        all_features = torch.transpose(all_features, 1, 3)
        features_map = F.softmax(all_features, dim=1)

        features1_weight = features_map[:, 0, :, :]
        features2_weight = features_map[:, 1, :, :]
        features3_weight = features_map[:, 2, :, :]
        features4_weight = features_map[:, 3, :, :]

        features1 = features1_weight * features1
        features2 = features2_weight * features2
        features3 = features3_weight * features3
        features4 = features4_weight * features4
        ...
        features_z_copy = torch.unsqueeze(feature_z, dim=1)
        features_xy_copy = torch.unsqueeze(feature_xy, dim=1)
        features_line_copy = torch.unsqueeze(feature_line, dim=1)

        all_pnet_features = torch.cat((features_z_copy, features_xy_copy, features_line_copy), dim=1)
        all_pnet_features = torch.transpose(all_pnet_features, 1, 3)
        all_pnet_features = self.conv2d_2(all_pnet_features)
        all_pnet_features = torch.transpose(all_pnet_features, 1, 3)
        pnet_features_map = F.softmax(all_pnet_features, dim=1)

        features_z_weight = pnet_features_map[:, 0, :, :]
        features_xy_weight = pnet_features_map[:, 1, :, :]
        features_line_weight = pnet_features_map[:, 2, :, :]

        feature_z = features_z_weight * feature_z
        feature_xy = features_xy_weight * feature_xy
        feature_line = features_line_weight * feature_line
        ...
        end_points['features_map'] = features_map
        end_points['pnet_features_map'] = pnet_features_map

        forward_end_time = time.time()
        forward_time = forward_end_time - forward_start_time
        print("forward_time: {}".format(forward_time))
        return end_points