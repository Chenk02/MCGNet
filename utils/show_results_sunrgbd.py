...
def export_one_scan(scan_name):
    ...
    pcd = o3d.io.read_point_cloud(mode+'tmp.xyzrgb')
    ...
    o3d.io.write_point_cloud('/home/ck/code/H3DNet/result/sunrgbd/vis_result/' + mode + '/' + scan_name + '_pcd.ply', pcd)

    pred_proposals = np.load(os.path.join(PRED_PATH, 'opt'+scan_name+'_nms.npy'))
    gt_bbox = sio.loadmat(os.path.join(PRED_PATH, 'opt'+scan_name+'_gt.mat'))['gt']
    bb =[]
    all_bbox = []
    if mode=='gt':
    ...
    with open('/home/ck/code/H3DNet/result/sunrgbd/vis_result/' + mode + '/' + scan_name + '_bbox.txt', 'w+') as f:
        for box in all_bbox:
            for v in box:
                f.write(str(v) + " ")
            f.write('\n')
